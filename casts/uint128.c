...
PG_FUNCTION_INFO_V1(uint16_from_uint1);
Datum uint16_from_uint1(PG_FUNCTION_ARGS) {
...
PG_FUNCTION_INFO_V1(uint16_to_uint1);
Datum uint16_to_uint1(PG_FUNCTION_ARGS) {
...
PG_FUNCTION_INFO_V1(uint16_from_int1);
Datum uint16_from_int1(PG_FUNCTION_ARGS) {
...
PG_FUNCTION_INFO_V1(uint16_to_int1);
Datum uint16_to_int1(PG_FUNCTION_ARGS) {
    uint128 a = PG_GETARG_UINT128(0);
...
PG_FUNCTION_INFO_V1(uint16_to_json);
Datum uint16_to_json(PG_FUNCTION_ARGS) {
    uint128 a = PG_GETARG_UINT128(0);
...
PG_FUNCTION_INFO_V1(uint16_to_jsonb);
Datum uint16_to_jsonb(PG_FUNCTION_ARGS) {
...
PG_FUNCTION_INFO_V1(uint16_from_json);
Datum uint16_from_json(PG_FUNCTION_ARGS)
{
    ...
    if (token != JSON_TOKEN_NUMBER)
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("cannot cast json type to type uint16")));
    }
    ...
    if (convRes == -1)
    {
        ereport(
            ...
            (
                ...
                errmsg("invalid input syntax for type uint16")
            )
        );
    }
...
Datum uint16_from_jsonb(PG_FUNCTION_ARGS)
{
    ...
    if (!JsonbExtractScalar(&in->root, &v))
        cannotCastJsonbValue(v.type, "uint16");
    ...
    if (convRes == -1)
    {
        ereport(
            ...
            (
                errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                errmsg("invalid input syntax for type uint16: \"%s\"", cStrValue)
            )
        );
    }
