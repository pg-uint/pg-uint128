...
PG_FUNCTION_INFO_V1(uint2_from_uint1);
Datum uint2_from_uint1(PG_FUNCTION_ARGS) {
...
PG_FUNCTION_INFO_V1(uint2_from_int1);
Datum uint2_from_int1(PG_FUNCTION_ARGS) {
...
Datum uint2_from_json(PG_FUNCTION_ARGS)
{
    ...
    if (token != JSON_TOKEN_NUMBER)
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("cannot cast json type to type uint2")));
    }

    /* Convert the text to C string */
    cStrValue = text_to_cstring(in);
    convRes = parse_uint16(cStrValue, &retValue);
    ...
    if (convRes == -1)
    {
        ereport(
            ...
            (
                ...
                errmsg("invalid input syntax for type uint2")
            )
        );
    }
    ...
    PG_RETURN_UINT16(retValue);
}

PG_FUNCTION_INFO_V1(uint2_from_jsonb);
Datum uint2_from_jsonb(PG_FUNCTION_ARGS)
{
    ...
    if (!JsonbExtractScalar(&in->root, &v))
        cannotCastJsonbValue(v.type, "uint2");
    ...
    if (convRes == -1)
    {
        ereport(
            ...
            (
                errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                errmsg("invalid input syntax for type uint2: \"%s\"", cStrValue)
            )
        );
    }
    ...
    if (convRes == -2)
    {
        ereport(
            ...
            (
                errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
                errmsg("uint2 out of range: %s", cStrValue)
            )
        );
    }
